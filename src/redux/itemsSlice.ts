...
import type { StateType, Transaction } from "../components/types";
...
const itemsSlice = createSlice({
  ...
  reducers: {
    ...
    updateItem: (
      state,
      action: PayloadAction<{ key: string; newData: Partial<Transaction> }>
    ) => {
...
export const selectAllItems = (state: RootState) =>
  state.transactionData.allTransactions;

export const selectItemById = (state: RootState, key: string) =>
  state.transactionData.allTransactions.find((item) => item.key === key);

export const { addItem, deleteItem, updateItem, readAllItems } =
  itemsSlice.actions;
